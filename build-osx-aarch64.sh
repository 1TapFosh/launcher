...
APPBASE="build/macos-aarch64/RuneFusion.app"
...
build() {
    ...
    cp native/build-aarch64/src/RuneFusion $APPBASE/Contents/MacOS/
    cp target/RuneFusion.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on RuneFusion
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/RuneFusion
    popd

    otool -l $APPBASE/Contents/MacOS/RuneFusion
}
...
dmg() {
    ...
    mv RuneFusion\ *.dmg RuneFusion-aarch64.dmg

    # dump for CI
    hdiutil imageinfo RuneFusion-aarch64.dmg

    if ! hdiutil imageinfo RuneFusion-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo RuneFusion-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit RuneFusion-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple RuneFusion-aarch64.dmg
    fi
}
